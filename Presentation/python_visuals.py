...
# 3×2 grid: columns = High/Med/Low volume, rows = Top 5 / Bottom 5
# ══════════════════════════════════════════════════════════════
display(Markdown("---\n## VIS-07: Outbound Condition Pathways (by Volume Tier)"))

dx = perf_dx.copy()
dx["tier"] = pd.cut(dx["trigger_volume"],
                     bins=[-1, 99, 999, dx["trigger_volume"].max() + 1],
                     labels=["Low (20–99)", "Med (100–999)", "High (1000+)"])
tiers = ["High (1000+)", "Med (100–999)", "Low (20–99)"]

fig, axes = plt.subplots(2, 3, figsize=(16, 7))

for col, tier in enumerate(tiers):
    subset = dx[dx["tier"] == tier].copy()
    top5 = subset.head(5)
    bot5 = subset.tail(5).iloc[::-1]

    for row, (chunk, clr, label) in enumerate([
        (top5, GREEN, "Top 5"),
        (bot5, RED, "Bottom 5"),
    ]):
        ax = axes[row][col]
        ...
        if n == 0:
            ...
            continue
        names = [d[:35] + "..." if len(d) > 35 else d for d in chunk["display"].values]
        ax.barh(range(n), chunk["hit_at_5"].values * 100,
                color=clr, edgecolor="white", height=0.6)
        ax.set_yticks(range(n))
        ax.set_yticklabels(names, fontsize=7)
        ax.invert_yaxis()
        for i, (v, vol) in enumerate(zip(chunk["hit_at_5"].values, chunk["trigger_volume"].values)):
            ax.text(max(v*100 + 0.5, 1), i, f"n={int(vol):,}",
                    va="center", fontsize=6, color=TEXT_MED)
        ax.set_xlim(0, 110)
        ax.xaxis.set_major_formatter(mticker.PercentFormatter())
        ax.grid(axis="x", color=GRID_CLR, linewidth=0.5)
        if row == 0:
        ...
        if row == 1:
            ax.set_xlabel("Hit@5 (%)", fontsize=8)

fig.suptitle("Outbound: Given this diagnosis, how often can we predict the next specialty?\n"
             "Top 5 & Bottom 5 per volume tier",
             fontsize=12, fontweight="bold", y=1.02)
plt.tight_layout()
plt.savefig(f"{OUT}vis_07_conditions_grid.png")
plt.show()
