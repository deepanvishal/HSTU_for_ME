...
-- ============================================================
-- TRACK 1 BASE — FP / I TRIGGERS ONLY
-- ============================================================
DROP TABLE IF EXISTS `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_f_track1_base_fp`;
CREATE TABLE `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_f_track1_base_fp`
OPTIONS (labels=[("owner", "deepan_thulasi_aetna_com")])
AS
WITH downstream_visits AS (
    SELECT DISTINCT
    ...
    LEFT JOIN `edp-prod-hcbstorage.edp_hcb_core_cnsv.ICD9_DIAGNOSIS` dx_desc
        ON f.dx_raw = dx_desc.icd9_dx_cd
    WHERE t.trigger_specialty IN ('FP', 'I')
)
SELECT
    member_id
    ,trigger_date
