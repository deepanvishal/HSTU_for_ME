-- ============================================================
-- Markov_trigger_scores.sql
-- Purpose : Roll up line-level Markov predictions to trigger level
--           Score against true labels — same schema as trigger_scores
--           Written to SEPARATE table A870800_gen_rec_markov_trigger_scores
-- Sources : A870800_gen_rec_markov_predictions  (DATE trigger_date, line-level rank 1-5)
--           A870800_gen_rec_markov_true_labels   (DATE trigger_date, ARRAY true_label_set)
-- Note    : trigger_date is DATE in both source tables — CAST to STRING on output
-- ============================================================

CREATE OR REPLACE TABLE `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_markov_trigger_scores`
OPTIONS (labels=[("owner", "deepan_thulasi_aetna_com")])
AS

WITH

-- ── Step 1: Roll up line-level predictions to ARRAY per trigger ───────────────
-- predicted_specialty, probability, prediction_rank confirmed from Model_Markov.sql
-- CAST trigger_date DATE → STRING here — used for all downstream joins
rolled AS (
    SELECT
        member_id
...
-- ── Step 2: True labels — cast DATE → STRING to match rolled ──────────────────
-- true_label_set is ARRAY<STRING>, true_label_count is INT64 — confirmed
labels AS (
    SELECT
        member_id
        ,CAST(trigger_date AS STRING)                    AS trigger_date
        ...
        ,true_label_set
        ,true_label_count
    FROM `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_markov_true_labels`
),

-- ── Step 3: Join on all 4 trigger keys, filter by window qualification ─────────
joined AS (
    SELECT
        r.member_id
        ,r.trigger_date
...
-- ── Step 4: Score each trigger-window row ─────────────────────────────────────
scored AS (
    SELECT
        member_id
        ,trigger_date
