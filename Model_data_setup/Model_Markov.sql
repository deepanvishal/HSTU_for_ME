...
WITH predictions_agg AS (
    ...
    SELECT
        ...
        ,ARRAY_AGG(predicted_specialty
            ORDER BY prediction_rank)                    AS predicted_specialties
...
joined AS (
    ...
    SELECT
        ...
        ,p.prediction_count
        ,CASE t.time_bucket
            WHEN 'T0_30'   THEN p.is_t30_qualified
            WHEN 'T30_60'  THEN p.is_t60_qualified
            WHEN 'T60_180' THEN p.is_t180_qualified
         END                                             AS is_window_qualified
    FROM `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_markov_true_labels` t
    LEFT JOIN predictions_agg p
        ON t.member_id = p.member_id
        AND t.trigger_date = p.trigger_date
        AND t.trigger_dx = p.trigger_dx
        AND t.member_segment = p.member_segment
    WHERE CASE t.time_bucket
            WHEN 'T0_30'   THEN p.is_t30_qualified
            WHEN 'T30_60'  THEN p.is_t60_qualified
            WHEN 'T60_180' THEN p.is_t180_qualified
          END = TRUE
...
per_trigger_k AS (
    -- Compute metrics per trigger per window per K
    SELECT
        member_id
        ,trigger_date
        ,trigger_dx
        ...
        -- TOP K predictions as array slice
        ,ARRAY(
            SELECT s FROM UNNEST(predicted_specialties) AS s WITH OFFSET pos
            WHERE pos < k_val
            ORDER BY pos
        )                                                AS top_k_preds

        -- Hits — specialties in both top K and true label set
        ,(
            SELECT COUNT(*)
            FROM UNNEST(predicted_specialties) AS pred WITH OFFSET pos
            WHERE pos < k_val
              AND pred IN UNNEST(true_label_set)
        )                                                AS hits_at_k
...
ORDER BY time_bucket, k_val, member_segment;
