...
-- Source  : A870800_gen_rec_sample_members_{X}pct
--           A870800_gen_rec_triggers_qualified
--           A870800_gen_rec_visits_qualified
--           A870800_gen_rec_model_input_sequences
-- Notes   : RL tables built from flat tables — no ARRAY joins
--           All tables independent per sample size
-- ============================================================


-- ══════════════════════════════════════════════════════════════════════════════
-- HELPER — SEQUENCE LABELS VIEW
-- Flat label rows from model_input_sequences
-- Used by model_train and model_test tables
-- Avoids reading ARRAY columns directly for joins
-- ══════════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE TABLE `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_model_labels_flat`
...
AS
SELECT l.*
FROM `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_model_labels_flat` l
INNER JOIN `anbc-hcb-dev.provider_ds_netconf_data_hcb_dev.A870800_gen_rec_sample_members_1pct` s
    ON l.member_id = s.member_id
WHERE l.trigger_date < DATE '2024-01-01'
  AND l.has_claims_12m_before = TRUE;
...
-- Built from flat tables — no ARRAY joins — no 3TB dependency
